/**
 * FarmRent - Common JavaScript Functions
 * Shared utilities used across all pages
 * Author: FarmRent Development Team
 */
 ...
 * @param {string} machineId - The ID of the machine to book
...
function saveBooking(machineId, formData) {
  ...
  const updatedMachines = machines.map(m => {
    if (m.id === machineId) {
      return {
        ...m,
        availableDates: m.availableDates.filter(d => d !== formData.bookingDate)
      };
    }
    return m;
  });
 ...
 * @param {string} status - The new status ('pending' or 'confirmed')
 */
 ...
 * @returns {boolean} True if valid password (min 6 chars)
 */
